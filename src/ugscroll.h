#ifndef GUI_UGSCROLL_H
#define GUI_UGSCROLL_H

// QPalette roles used by UGScroll:
// - Button: center monitor and idle button background.
// - Accent: pressed/hovered buttons.
// - ButtonText (Normal/Disabled): entry text and grayed color.
 ...
 *
 *  Grayed entries will be displayed using the grayed color (see QPalette usage below) in default
 * configuration. If hideGrayed property has been set, grayed entries will be hidden instead of showed with
 * grayed color.
 *
 ...
 *
 *      -QPalette::Button
 *          Used for center monitor and idle button background
 *
 *      -QPalette::Accent
 *          Used for button hovering/clicked color
 *
...
namespace gui
{
    class ULTRAGUI_EXPORT UGScroll : public QWidget
    {
        Q_OBJECT
       ...
       public:
        explicit UGScroll(QWidget* parent = nullptr);
